...
if [ -f dist/shellsmith-1.5b-f16.gguf ]; then
  hf upload "$MODEL_REPO" dist/shellsmith-1.5b-f16.gguf shellsmith-1.5b-f16.gguf --repo-type model
fi
...
hf upload "$DATA_REPO" data/test.jsonl  test.jsonl  --repo-type dataset
hf upload "$DATA_REPO" /tmp/_ds_readme.md README.md --repo-type dataset
